...
SELECT car.marca, car.modelo, SUM(via.distanciaPercorrida) AS distancia_percorrida
FROM carro car, viagem via, empresa emp
WHERE car.idEmpresa = emp.id
	AND car.id = via.idCarro
GROUP BY car.marca, car.modelo
ORDER BY SUM(via.distanciaPercorrida)

--Consultar Nomes das empresas que não tem carros cadastrados
SELECT emp.nome
FROM empresa emp
LEFT OUTER JOIN carro car
...
SELECT car.marca, car.modelo, COUNT(v.idCarro) AS qtd_viagens
FROM carro car, viagem v, empresa emp
WHERE car.idEmpresa = emp.id
	AND v.idCarro = car.id
GROUP BY car.marca, car.modelo, emp.nome
ORDER BY emp.nome ASC, COUNT(v.idCarro) DESC
...
SELECT emp.nome, car.modelo, via.distanciaPercorrida,
	CASE
		WHEN via.distanciaPercorrida < 1000 THEN
			via.distanciaPercorrida * 10
		ELSE
			via.distanciaPercorrida * 15
	END AS total_valor_ganho_viagem
FROM empresa emp, carro car, viagem via
WHERE emp.id = car.idEmpresa
	AND via.idCarro = car.id